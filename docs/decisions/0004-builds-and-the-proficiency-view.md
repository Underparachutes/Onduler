...
**Amended:** 2026-05-22 — see ADR 0011. The user-facing word **shape** is dropped. The Settings tile renames from "Your shape" to **Starter sets**; the four presets (Maker / Athlete / Wanderer / Scholar) survive as utility-flavored starter sets, not identity claims. The secondary slot drops from UI (the `secondary_build` column stays for possible re-introduction). Welcome-back copy in §8 rewords from "Pick up your [shape]" to "Pick up where you left off"; "Try a different shape →" becomes "Try a different starter set →". Internal `build` identifiers (`primary_build`, `secondary_build`, `lib/builds.ts`, `BUILD_PRESETS`) stay unchanged — same internal/external split established by ADR 0006 (milestone/Waypoint) and ADR 0008 (reflection/Anchor). The build context footer ("Part of your maker's rhythm") on the per-swell proficiency view is removed.
