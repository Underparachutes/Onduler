...
- **`prf` support — FULLY CONFIRMED (2026-06-17).** Spike passed on iPhone iOS 26 (WebKit) and desktop Chrome/macOS: `prf.enabled = true`, two reads identical. **Cross-device sync also confirmed** — the iCloud-synced passkey produced a byte-identical `prf` secret on iPhone and Mac Safari (both under `onduler.app`), so recovery rides on iCloud for the common case and the "passkey = invisible backup" claim holds. Passkey-primary is locked for v1; recovery code + password fallback are the edge-case safety nets. (Throwaway test page `public/passkey-test.html` removed post-verification.)
