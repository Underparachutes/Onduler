...
// Sunday 00:00 Pacific of the current week (Sun–Sat cycle)
export async function getWeekStart(): Promise<Date> {
  return pacificMidnightOf(sundayOf(pacificToday()))
}

// Sunday 00:00 Pacific of the previous week
export async function getLastWeekStart(): Promise<Date> {
  return pacificMidnightOf(sundayOf(pacificToday(), 1))
}
