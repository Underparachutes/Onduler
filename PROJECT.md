...
| Session | Goal |
|---|---|
| **Next** | Reflections surface (ADR 0007) — rename Log → Reflections and Today → Motions (checkbox icon); first-time-only cycle-close unlocks (weekly / monthly / quarterly / yearly) with calendar anchoring + engagement floors (3 / 8 / 25 / 80 days); vibe-only locked-page anticipation surface (blurred radar silhouette, tide-line motion, soft copy, no date, no engagement counter); cycle-close ceremony fired from the bottom-nav tab itself (other tabs dim, Reflections animates); ceremony is one shape across all four cadences — *"What did you expect to see this [cycle]?"* → closed-cycle radar reveal → *"What did you see?"* → *"Want to tune something?"* with Swells / Motions / Skip; both prompts optional and skippable; wave-cycle handling (no ceremony, no indicator, soft "we'll be here when you're back on the board"); persistent reflections journal at `/reflections/journal` as a contemplative library sub-surface (paper-list aesthetic, chapter separators, all chapters' reflections live together here); `+` opens a blank free-form journal entry (default-anchored to currently-viewed cycle, optional themed prompts from the recycled MI question bank); archive-and-fresh-start available only at the end of a cycle-close ceremony (reward reflection with a clean slate — full re-onboarding, new chapter re-earns unlocks, past chapters preserved read-only in Settings → Past chapters); after-year-one user-chosen cadence preference (deferred dependency on a notification system). Schema additions: `chapters` and `reflections` tables. Helpers extend `lib/periods.ts` for quarterly. |
