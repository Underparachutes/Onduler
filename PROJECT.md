...
| Session | Goal |
|---|---|
| **Next** | Swells page rebuild → per-swell proficiency view. Constellation of motions feeding the swell (size = contribution weight, opacity = recent activity, log-count inside each node), arrangeable via drag, capped at 8 visible with a "+ N more" overflow to a flat list, list-view toggle in the top-right. Header keeps weekly tide bar + small lifetime stat line (no tier badges). Time-view toggle (Week / Month / Lifetime). Milestones section below (recurring + one-shot, sortable, completed collapse by default). Schema additions: `motion_swells.position_x/y` and the `milestones` table. See ADR 0004. |
| **After** | Settings build picker — "Your shape" surface with current shape + secondary-shape slot (multi-class entry, slot only) + preset gallery + "Build your own" path. Tapping a preset opens the preview/diff screen with per-change opt-out checkboxes. Nothing deleted; swells hidden at most. Adds `user_settings.primary_build` and `user_settings.secondary_build`. See ADR 0004. |
| **After** | Normalized contribution model — ship the math change (motion's contributions across swells sum to ≤100%, auto-rebalance on edit, allocation indicator in motion detail sheet). One-time renormalization of existing `motion_swells` rows. Pairs naturally with the build picker since build-suggested motions emit per-swell splits that depend on this model. See ADR 0003. |
| **After** | Logs page redesign — radar/hexagon comparison of actual week against build's target shape. Softened/Reference lens toggle, with softened as the default during/after a wave. Wave windows marked as context, not failure. Multi-classing math lands here (secondary shape's targets blend into one overall shape the user orients toward). See ADR 0004. |
| **After** | Wave / build interaction — welcome-back screen after WaveGrid check-in with two doors (Ease back in / Pick up your [shape]) plus a small "Try a different shape" link. Auto-selected minimum-viable-shape (top 2 most-logged motions per shape) with Settings affordance to drop, swap, or add anchors. See ADR 0004. |
