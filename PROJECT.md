...
| Session | Goal |
|---|---|
| **Next** | `swells.hidden` flag + UI — let users hide a swell when focus shifts without losing logs or motions; quiet "Hidden swells" section to restore. Paired with whatever surface first needs it (likely the build picker once we want presets to suggest hiding swells the shape doesn't seed). |
