...
Sessions A+B+C of the Reflections surface (ADR 0007) shipped May 18 2026 — rename, schema, weekly cycle-close ceremony, engagement-floor unlocks, locked-page. Josh is intentionally living with that for a stretch before resuming. The remainder of the Reflections work is queued below.

| Session | Goal |
|---|---|
| **Reflections D** | Persistent reflections journal at `/reflections/journal` — contemplative-library sub-surface listing every reflection across all chapters, in chronological order with chapter separators. Paper-list aesthetic; each entry card shows the closed-radar visual (ceremony entries) or body text + optional prompt header (free entries). `+` button in the top-right of `/reflections` opens a blank free-form entry (`cycle_type='free'`, default-anchored to the currently-viewed cycle, editable). Optional "start with a prompt" affordance pulls from a themed bank (the recycled MI questions, organized by theme — notice / honor / consider / release / invite). |
| **Reflections E** | Archive-and-fresh-start — only accessible at the end of a completed cycle-close ceremony (as a third option below the Swells / Motions / Skip CTAs). Confirming runs a soft warning then ends the active chapter (`chapters.ended_at = now()`) and starts a new one with `started_at = now()`. Past chapters browseable from Settings → Past chapters (read-only). Open question: schema-scope existing tables to chapters (`chapter_id` FK on motions/swells/logs/etc., approach (a)) vs. snapshot to JSON on archive (approach (b)). Decide at session start. |
| **Reflections F** | Monthly / quarterly / yearly cycle-close ceremony routes — `/reflections/ceremony/month`, `/reflections/ceremony/quarter`, `/reflections/ceremony/year`. Same one-shape ceremony (expectation → reveal → observation → tune) as weekly. Pending banner and bottom-nav signal extend to fire when any cadence has a pending closed cycle. Quarterly target = `ceil(weekly × days_in_quarter / 7)`, yearly = `ceil(weekly × days_in_year / 7)`. |
| **Admin account** | Admin role for Josh (and any future Onduler operator) — bypass RLS to view user counts, recent signups, basic per-user data for debugging or support, possibly a switch-to-user view. Likely an `is_admin boolean` on `user_settings` or a separate `admins` table; a `/admin` route that's gated by the flag. Tight scope: read-only diagnostics first, no destructive ops in v1. |
...
ADR: `docs/decisions/0007-reflections-surface.md`.
...
- **Bottom nav is the primary navigation on mobile; back is for sub-routes only.** Top-level pages reached via bottom nav (Motions, Swells, Reflections, Settings) hide their back button at mobile widths — users navigate via bottom nav. Sub-routes (motion detail sheet, inline edit forms, Add Motion/Swell/Group, Settings group-edit, `/reflections/journal`) keep their back/cancel because there's no bottom-nav route back to them. Desktop shows back everywhere; mobile shows back only where the bottom nav can't get you back. (Motions and Reflections are formerly Today and Log — see ADR 0007.)
...
- **Motions is the daily ritual surface; Swells is the strategic surface; Reflections is the mirror where you adjust yourself; Settings is configuration.** Motions stays paper-light because it's where the user shows up every day to tap motion boxes and move on — any structural complexity introduced here would break the ritual. Swells is where structural depth belongs: per-swell proficiency views, motion constellations, waypoints, time-view zooms, build context. Reflections is where the user notices and changes — cycle-close ceremonies, the radar with drag-to-tune, the persistent journal, and the only path to archive-and-fresh-start. Settings carries one-time configuration. The user goes to Swells to *tune their shape*; they go to Motions to *do the work*; they go to Reflections to *notice and change*; they go to Settings to *configure*. This four-surface role split — ritual / strategy / reflection / configuration — is how Onduler scales as users invest more without ever becoming exhausting. New features get evaluated against this split — "which surface does this belong on, by role?" is a real question. (Motions / Reflections are formerly Today / Log — see ADR 0007. See also ADR 0004 for the original Today / Swells split rationale.)
