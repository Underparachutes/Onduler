...
| Session | Goal |
|---|---|
| **Next** | Calendar-month view on the Log page + proficiency view — wire the radar to the period filter so chart reflects "this week / this month / all time" (currently chart stays fixed on this-week regardless of which filter button is active). ADR 0005 covers the math: calendar-anchored cycle, monthly target = `ceil(weekly × days_in_month / 7)` computed at render, weekly target stays the integer source of truth, wave month = 7+ consecutive zero-log days, lifetime view is read-only (no drag). Symmetric drag editability — drag on monthly view edits weekly underneath with appropriate step scaling; live pill shows both values (`Mind · 222 pts/mo · 50 pts/wk`). `ceil` display rule applies everywhere a number renders, not just the radar. Pair fix rides along: lifetime stat shows absolute total when `weeks_since_first_log ≤ 1`. Filter labels rename ("7 days" → "This week", "30 days" → "This month"). Pure helpers in `lib/periods.ts` first, then thread `period` prop through the radar component, then update proficiency view Month tab to use the same helpers. See ADR 0005. |
...
- **Ceil display rule — no decimals reach the screen.** Every number that renders (targets, actuals, ratios, percentages) is `ceil`'d to whole units. Underlying state stays precise — radar polygon geometry uses unrounded floats so vertices land in the right pixel — but anywhere a user sees a number, it's an integer. `ceil` rather than `round` because the slight upward bias matches the celebration-leaning posture (target is slightly higher than the true float; actuals are slightly higher than the true sum). Applies app-wide, not just the radar. See ADR 0005.
