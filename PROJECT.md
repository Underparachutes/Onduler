...
| **After** | Waypoints copy sweep + export translation — visible-surface rename of "Milestone(s)" → "Waypoint(s)" across the proficiency view section header, empty states, inline labels, completed-collapse text ("N finished milestones ↓" → "N finished waypoints ↓"), and any create/edit form copy. Schema stays `milestones`; code identifiers (`MilestonesSection.tsx`, `app/actions/milestones.ts`, etc.) stay unless a rename is cheap and improves clarity. Pair with the `/api/export` translation: include waypoints in the export (currently missing entirely), and translate top-level keys to user-facing vocabulary (`milestones` → `waypoints`; audit any other schema-shaped keys like `motion_swells` and decide what surfaces). Filename + `exported_at` + `user_email` stay as-is — those are export-format metadata, not user-data labels. See ADR 0006. |
